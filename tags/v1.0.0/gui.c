                 ...
                 int blocks_with_excess_byte)
{
	...
	int i;
	char block_one[bytes_per_block], block_two[bytes_per_block];
	for (i = 0; i < total_blocks; i++) {
		...
		// character. This is a fairly crappy approach, but the standard
		// block_one[j] != block_two[j] to compare bytes produces wrong 
		// results.
			
		int j;
		char block_hex_one[12],block_hex_two[12]; 
		for (j = 0; j < bytes_in_block; j++) {
			sprintf(block_hex_one, "%02x", block_one[j]);
			block_hex_one[2] = '\0';
			sprintf(block_hex_two, "%02x", block_two[j]);
			block_hex_two[2] = '\0';
			if (strcmp(block_hex_one, block_hex_two) != 0) {
				block_cache[i] = BLOCK_DIFFERENT;
				break;
			}
		}
